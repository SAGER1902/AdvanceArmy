...
{
	public ResourceLocation tex = new ResourceLocation("advancearmy:textures/mob/soldier/minigunner.png");
	...
	public ResourceLocation star = new ResourceLocation("advancearmy:textures/mob/soldier/star.png");
    public RenderGAT(EntityRendererManager renderManagerIn)
    {
    ...
    float iii;
    public void render(EntitySA_GAT entity, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
		...
		if(entity.getVehicle()!=null){
			...
			if(entity.getSame()){
				Minecraft.getInstance().getTextureManager().bind(star);
    ...
    private void renderlegs(EntitySA_GAT entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderbody(EntitySA_GAT entity, float limbSwing, float limbSwingAmount, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn){
		...
		GL11.glPushMatrix();//glstart
		{
			...
			if(entity.isAttacking() && !(entity.deathTime > 0) && !(entity.getRemain1()==0)||entity.sit_aim && !entity.isPassenger()) {
				if(entity.anim1<5){
	    			GL11.glRotatef(-entity.anim1*0.5F, 1.0F, 0.0F, 0.0F);
	    		}
			...
			if(entity.canfire){
				...
				if(!entity.isAttacking()){
					...
					GL11.glRotatef(81F, 0.0F, 1.0F, 0.0F);
					GL11.glRotatef(-79F, 0.0F, 0.0F, 1.0F);
					GL11.glTranslatef(-1.9F, 3.3F, -3F);//x,z,y
				}else{
					GL11.glRotatef(90F, 1.0F, 0.0F, 0.0F);
					GL11.glTranslatef(-1.9F, -0.2F, -4.35F);//x,z,y
				}
				...
				SARenderHelper.enableBlendMode(RenderTypeSA.ADDITIVE);//
				GL11.glTranslatef(-entity.fireposX, -0.1F, 1.1F);
