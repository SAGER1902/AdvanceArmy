...
{	
    ...
    private static final ResourceLocation gun2 = new ResourceLocation("advancearmy:textures/gun/m16a4.png");
    ...
    private static final ResourceLocation at4t = new ResourceLocation("advancearmy:textures/gun/at4.png");
    private static final SAObjModel javelin = new SAObjModel("advancearmy:textures/gun/javelin.obj");
    private static final ResourceLocation javelint = new ResourceLocation("advancearmy:textures/gun/javelin.png");
    ...
    public RenderSoldier(EntityRendererManager renderManagerIn)
    {
    ...
    private void render_light(EntitySA_Soldier entity, String name){
    ...
    private void render_part(EntitySA_Soldier entity, String name) {
        ...
        if (entity.soldierType == 1){
			...
			Minecraft.getInstance().getTextureManager().bind(tex4);
			obj0.renderPart(name);
        }else{
			Minecraft.getInstance().getTextureManager().bind(tex0);
			obj0.renderPart(name);
		}
    ...
    float iii;
    public void render(EntitySA_Soldier entity, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
    ...
    private void renderlegs(EntitySA_Soldier entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderbody(EntitySA_Soldier entity, float limbSwing, float limbSwingAmount, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn){
		...
		if(entity.getWeaponId()==8){
			wait_model = at4;
			minecraft.getTextureManager().bind(at4t);
		}
		if(entity.getWeaponId()==9){
			wait_model = javelin;
			minecraft.getTextureManager().bind(javelint);
		}
		...
		if(entity.mainWeaponId!=0){
			...
			/*GL11.glRotatef(entity.roteXanim, 1.0F, 0.0F, 0.0F);
			GL11.glRotatef(entity.roteYanim, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(entity.roteZanim, 0.0F, 0.0F, 1.0F);
			GL11.glTranslatef(entity.posXanim, entity.posYanim, entity.posZanim);*///x,z,y
				...
				/*if(heldItem.isEmpty())*/{
					...
					if(heldItem.getItem() instanceof GunItem && (Gun.hasAmmo(heldItem))){
						...
						if(modifiedGun != null)
						{
							...
							matrixStackIn.pushPose();
							if(entity.getRemain2()==1){
								matrixStackIn.translate(0F, 0.15F, -1.5F);
								matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(90F));
							}
							
							matrixStackIn.mulPose(Vector3f.YP.rotationDegrees(180.0F - entity.yBodyRot));
							
							matrixStackIn.translate(0.37F, 1.375F, 0.0F);
							
							if(entity.anim1<5){
								matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(entity.anim1));
							}
							
							if(entity.getRemain2()==1){//
								matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(-arm_r_rotex+180F));
							}else{
								matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(-arm_r_rotex));
							}
							matrixStackIn.mulPose(Vector3f.YP.rotationDegrees(arm_r_rotey));
							matrixStackIn.mulPose(Vector3f.ZP.rotationDegrees(-arm_r_rotez));
							matrixStackIn.translate(-0.37F, -1.375F, 0.0F);
							
							matrixStackIn.pushPose();//
							matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(-90F));
							matrixStackIn.translate(0.37F, 0.5F, 0.5F);
							boolean right = true;
							...
							if(ModelOverrides.hasModel(heldItem))
							{
								...
								if(model != null)
								{
									model.render(partialTicks, transformType, heldItem, ItemStack.EMPTY, entity, matrixStackIn, bufferIn, 240, OverlayTexture.NO_OVERLAY);
								}
							}else
							{
								IBakedModel bakedModel = minecraft.getItemRenderer().getItemModelShaper().getItemModel(heldItem);
								minecraft.getItemRenderer().render(heldItem, ItemCameraTransforms.TransformType.NONE, false, matrixStackIn, bufferIn, 240, OverlayTexture.NO_OVERLAY, bakedModel);
							}
							matrixStackIn.popPose();//
							...
							minecraft.getTextureManager().bind(fire_tex);
							SARenderHelper.enableBlendMode(RenderTypeSA.ADDITIVE);//
							GL11.glTranslatef(-0.36F, 0.23F, 0.5F);
							float size = entity.level.random.nextInt(3) * 0.25F + 1;
							GlStateManager._scalef(size, size, size);
							if(entity.anim1<3 && entity.getRemain1() > 0 && entity.isAttacking())
							{
