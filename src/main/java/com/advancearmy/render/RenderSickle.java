...
{
	private static final ResourceLocation tex = new ResourceLocation("advancearmy:textures/mob/sickle.png");
    ...
    public RenderSickle(EntityRendererManager renderManagerIn)
    {
	...
	public boolean shouldRender(EntitySA_Sickle p_225626_1_, ClippingHelper p_225626_2_, double p_225626_3_, double p_225626_5_, double p_225626_7_) {
      return true;
   }
   ...
   private static void setupGlintTexturing(float p_228548_0_) {
    ...
    private void render_part(EntitySA_Sickle entity, String name){
		{
	...
	public void renderleg(int id,EntitySA_Sickle entity, float x, float y, float z, String name, float partialTicks, float hox, float hoy, float hoz, float level,
	...
	static int tick2 = 0;
	static int tick3 = 0;
	private void render_turret(EntitySA_Sickle entity, String name, float x, float y, float z, float rote, float pitch, float fire, float tick, int id){
		GL11.glPushMatrix();//glstart
		GL11.glTranslatef(x, y, z);//
		...
		//GL11.glPushMatrix();//glstart
		//GL11.glTranslatef(x, y, z);
		if(id!=2)render_part(entity,name);
		//GL11.glPopMatrix();//glend
		
		GL11.glTranslatef(x, y, z+0.44F);//
		...
		if(id==3){
			...
			GL11.glTranslatef(entity.turretX, y, 0.8F);//
			if(fire < 3){
	...
	static int shock =0;
    public void render(EntitySA_Sickle entity, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
		...
		}
		{
			...
			if(entity.hurtTime>0){
				...
				GL11.glRotatef(-this.shock * count/50F, 1.0F, 0.0F, 0.0F);
				GL11.glRotatef(-this.shock * count/60F, 0.0F, 1.0F, 0.0F);
			}
			GL11.glRotatef(-(180.0F - entity.turretYaw), 0.0F, 1.0F, 0.0F);
		}
...
//倾斜系统
		...
		renderleg(1,entity, -0.67F, 1F, 0.65F, "leg1", partialTicks, 1, -1F, 0F, 0.3F, -1.66F,1.72F,1.06F);
		renderleg(1,entity, -0.67F, 1F, -0.67F, "leg2", partialTicks, -1, 1F, 0F, 0.3F, -1.66F,1.72F,-1.06F);
		
		renderleg(2,entity, 0.67F, 1F, 0.65F, "leg3", partialTicks, 1, -1F, 0F, 0.4F, 1.66F,1.72F,1.06F);
		renderleg(2,entity, 0.67F, 1F, -0.67F, "leg4", partialTicks, -1, 1F, 0F, 0.4F, 1.66F,1.72F,-1.06F);
		...
		render_turret(entity, "turret", 0F, 3.11F, 0F, rote1, (entity.xRotO + (entity.xRot - entity.xRotO) * partialTicks), 0, 0, 1);
		...
		render_turret(entity, "rote1", -1.25F, 2.83F, 0F, 0, (entity.xRotO + (entity.xRot - entity.xRotO) * partialTicks), entity.anim1, tick1, 2);
		render_turret(entity, "rote2", 1.25F, 2.83F, 0F, 0, (entity.xRotO + (entity.xRot - entity.xRotO) * partialTicks), entity.anim1, tick1, 2);

		render_turret(entity, "turret2", 0, 4.04F, -0.54F, rote2, entity.turretPitch_1, entity.anim3, tick2, 3);
