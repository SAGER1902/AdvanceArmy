...
public class SoldierMachine extends TameableEntity implements IHealthBar, IBuilding{
	public SoldierMachine(EntityType<? extends SoldierMachine> p_i48549_1_, World p_i48549_2_) {
	  super(p_i48549_1_, p_i48549_2_);
	  this.noCulling = true;
	}
	public SoldierMachine(FMLPlayMessages.SpawnEntity packet, World worldIn) {
	...
	public SoldierMachine getBreedOffspring(ServerWorld p_241840_1_, AgeableEntity p_241840_2_) {
	  return null;
	}
   
	private static final DataParameter<Integer> VehicleC = EntityDataManager.<Integer>defineId(SoldierMachine.class, DataSerializers.INT);
	private static final DataParameter<Integer> Money = EntityDataManager.<Integer>defineId(SoldierMachine.class, DataSerializers.INT);
	private static final DataParameter<Integer> Build = EntityDataManager.<Integer>defineId(SoldierMachine.class, DataSerializers.INT);
	private static final DataParameter<Integer> YawRote = EntityDataManager.<Integer>defineId(SoldierMachine.class, DataSerializers.INT);
				...
				if(heldItem.getItem() instanceof PickaxeItem){
					if(player.isCrouching()){
						if(!this.level.isClientSide && this.getHealth()>=this.getMaxHealth()){
							this.dropItemStack(new ItemStack(AdvanceArmy.soldier_machine));
					 ...
					 if(item instanceof ItemSpawn){
						...
						if(this.getVehicleC()==0){
							if(vehicleitem.type == 1){
								...
								player.sendMessage(new TranslationTextComponent("advancearmy.interact.machines.desc", new Object[0]), player.getUUID());
							...
							/*if(player != null && this.isOwner(player))*/{
								if(this.getMoney()>0){
									if(this.getBuild() < this.finish_time){
										...
										if(this.getBuild()%20==0)this.playSound(SoundEvents.PORTAL_AMBIENT, 2.0F, 1.0F);
									}
								}
							}
						}
